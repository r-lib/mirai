...
- **daemons.R**:
  [`daemons()`](https://mirai.r-lib.org/reference/daemons.md), remote
  configs, compute profiles,
...
- **dispatcher.R**: Dispatcher process (thin wrapper — core logic now in
  C via nanonext)
- **map.R**:
  [`mirai_map()`](https://mirai.r-lib.org/reference/mirai_map.md) with
  collection options
- **launchers.R**:
...
- **next.R**: Developer interface
  ([`nextstream()`](https://mirai.r-lib.org/reference/nextstream.md),
  [`nextget()`](https://mirai.r-lib.org/reference/nextstream.md)) for
  packages extending mirai
