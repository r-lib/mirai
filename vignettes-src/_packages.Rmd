...
1. [Using mirai in a Package](#using-mirai-in-a-package)
...
2. The function `daemons_set()` may be used to detect if daemons have already been set and prompt the user to set daemons if not.

3. The function `on_daemon()` may be used to detect if code is running on a daemon, i.e. within a `mirai()` call.

4. The shape and contents of a `status()` call must not be used programatically, as this user interface is subject to change at any time.
- There is a developer interface `nextget()`, for querying values such as 'urls' described in the function's documentation.
  Note: only the specifically-documented values are supported interfaces.

5. The functions `unresolved()`, `is_error_value()`, `is_mirai_error()`, and `is_mirai_interrupt()` should be used to test for the relevant state of a mirai or its value.
...
6. Testing on CRAN should respect it's 2-core usage limit.
