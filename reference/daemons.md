...
- url:
  ...
  [`host_url()`](https://mirai.r-lib.org/reference/host_url.md) may be
  used to construct a valid host URL.

- remote:

  (required only for launching remote daemons) a configuration generated
  by [`ssh_config()`](https://mirai.r-lib.org/reference/ssh_config.md),
...
- ...:
  ...
  [`daemon()`](https://mirai.r-lib.org/reference/daemon.md) if launching
  daemons. These include `asyncdial`, `autoexit`, `cleanup`, `output`,
  `maxtasks`, `idletime`, `walltime` and `tlscert`.
...
- [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) reverts to the
  default behaviour of creating a new background process for each
  request.
...
[`daemon()`](https://mirai.r-lib.org/reference/daemon.md) processes are
automatically launched on the local machine connecting back to the host
process, either directly or via dispatcher.
...
[`daemon()`](https://mirai.r-lib.org/reference/daemon.md) processes dial
in to this URL. Host / dispatcher automatically adjusts to the number of
daemons actually connected, allowing dynamic upscaling / downscaling.
...
[`status()`](https://mirai.r-lib.org/reference/status.md) to inspect the
actual assigned port at any time.
...
Subsequent [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) calls
may then be sent for local computation by not specifying the `.compute`
...
[`mirai()`](https://mirai.r-lib.org/reference/mirai.md) calls, tasks may
be sent to either `cpu` or `gpu` daemons as appropriate.
