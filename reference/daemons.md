...
- ...:
  ...
  [`daemon()`](https://mirai.r-lib.org/reference/daemon.md) when
...
[`mirai()`](https://mirai.r-lib.org/reference/mirai.md) calls may then
be sent for local computation by not specifying the `.compute` argument,
or for remote computation to connected daemons by specifying the
`.compute` argument as `"remote"`.
