# Package index

## Async Evaluation

- [`mirai()`](https://mirai.r-lib.org/reference/mirai.md)
  [`try_mirai()`](https://mirai.r-lib.org/reference/mirai.md) : mirai
  (Evaluate Async)
- [`mirai_map()`](https://mirai.r-lib.org/reference/mirai_map.md) :
  mirai Map
- [`call_mirai()`](https://mirai.r-lib.org/reference/call_mirai.md) :
  mirai (Call Value)
- [`collect_mirai()`](https://mirai.r-lib.org/reference/collect_mirai.md)
  : mirai (Collect Value)
- [`race_mirai()`](https://mirai.r-lib.org/reference/race_mirai.md) :
  mirai (Race)
- [`stop_mirai()`](https://mirai.r-lib.org/reference/stop_mirai.md) :
  mirai (Stop)
- [`unresolved()`](https://mirai.r-lib.org/reference/unresolved.md) :
  Query if a mirai is Unresolved
- [`everywhere()`](https://mirai.r-lib.org/reference/everywhere.md) :
  Evaluate Everywhere

## Daemon Infrastructure

- [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) : Daemons
  (Set Persistent Processes)
- [`launch_local()`](https://mirai.r-lib.org/reference/launch_local.md)
  [`launch_remote()`](https://mirai.r-lib.org/reference/launch_local.md)
  : Launch Daemons
- [`daemon()`](https://mirai.r-lib.org/reference/daemon.md) : Daemon
  Instance

## Remote Configuration

- [`remote_config()`](https://mirai.r-lib.org/reference/remote_config.md)
  : Generic Remote Launch Configuration
- [`ssh_config()`](https://mirai.r-lib.org/reference/ssh_config.md) :
  SSH Remote Launch Configuration
- [`cluster_config()`](https://mirai.r-lib.org/reference/cluster_config.md)
  : Cluster Remote Launch Configuration
- [`http_config()`](https://mirai.r-lib.org/reference/http_config.md) :
  HTTP Remote Launch Configuration
- [`host_url()`](https://mirai.r-lib.org/reference/host_url.md)
  [`local_url()`](https://mirai.r-lib.org/reference/host_url.md) : URL
  Constructors

## Compute Profiles

- [`info()`](https://mirai.r-lib.org/reference/info.md) : Information
  Statistics
- [`status()`](https://mirai.r-lib.org/reference/status.md) : Status
  Information
- [`daemons_set()`](https://mirai.r-lib.org/reference/daemons_set.md) :
  Query if Daemons are Set
- [`require_daemons()`](https://mirai.r-lib.org/reference/require_daemons.md)
  : Require Daemons
- [`with_daemons()`](https://mirai.r-lib.org/reference/with_daemons.md)
  [`local_daemons()`](https://mirai.r-lib.org/reference/with_daemons.md)
  : With Daemons
- [`with(`*`<miraiDaemons>`*`)`](https://mirai.r-lib.org/reference/with.miraiDaemons.md)
  : With Mirai Daemons

## Serialization

- [`serial_config()`](https://mirai.r-lib.org/reference/serial_config.md)
  : Create Serialization Configuration
- [`register_serial()`](https://mirai.r-lib.org/reference/register_serial.md)
  : Register Serialization Configuration

## Promises

- [`as.promise(`*`<mirai>`*`)`](https://mirai.r-lib.org/reference/as.promise.mirai.md)
  : Make mirai Promise
- [`as.promise(`*`<mirai_map>`*`)`](https://mirai.r-lib.org/reference/as.promise.mirai_map.md)
  : Make mirai_map Promise

## Parallel Clusters

- [`make_cluster()`](https://mirai.r-lib.org/reference/make_cluster.md)
  [`stop_cluster()`](https://mirai.r-lib.org/reference/make_cluster.md)
  : Make Mirai Cluster

## Utilities

- [`is_mirai()`](https://mirai.r-lib.org/reference/is_mirai.md)
  [`is_mirai_map()`](https://mirai.r-lib.org/reference/is_mirai.md) : Is
  mirai / mirai_map
- [`is_mirai_error()`](https://mirai.r-lib.org/reference/is_mirai_error.md)
  [`is_mirai_interrupt()`](https://mirai.r-lib.org/reference/is_mirai_error.md)
  [`is_error_value()`](https://mirai.r-lib.org/reference/is_mirai_error.md)
  : Error Validators
- [`on_daemon()`](https://mirai.r-lib.org/reference/on_daemon.md) : On
  Daemon

## Package

- [`mirai-package`](https://mirai.r-lib.org/reference/mirai-package.md)
  : mirai: Minimalist Async Evaluation Framework for R
