...
- n:
  ...
  (integer) number of daemons to launch. For `launch_remote()`, may also
  be a 'miraiCluster' or 'miraiNode'.

- ...:

  (daemon arguments) passed to
  [`daemon()`](https://mirai.r-lib.org/reference/daemon.md), including
  `asyncdial`, `autoexit`, `cleanup`, `output`, `maxtasks`, `idletime`,
  and `walltime`. Overrides arguments from
  [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) if
  supplied.
