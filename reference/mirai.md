...
[`unresolved()`](https://mirai.r-lib.org/reference/unresolved.md) may be
used on a mirai, returning TRUE if a 'mirai' has yet to resolve and
FALSE otherwise. This is suitable for use in control flow statements
such as `while` or `if`.

Alternatively, to call (and wait for) the result, use
[`call_mirai()`](https://mirai.r-lib.org/reference/call_mirai.md) on the
...
[`daemons()`](https://mirai.r-lib.org/reference/daemons.md)), otherwise
leave as `"default"`.
