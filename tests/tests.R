...
connection && requireNamespace("otel", quietly = TRUE) && Sys.getenv("NOT_CRAN") == "true" && {
  ...
  length(record) && {
    ...
    test_equal(traces[[1L]]$attributes$rpc.system, "nng")
    test_equal(traces[[1L]]$attributes$server.address, purl[["hostname"]])
    ...
    test_equal(traces[[1L]]$attributes$mirai.compute.profile, "default")
    ...
    test_equal(traces[[8L]]$attributes$rpc.system, "nng")
    test_equal(traces[[8L]]$attributes$server.address, purl[["hostname"]])
    ...
    test_equal(traces[[14L]]$attributes$rpc.system, "nng")
    test_equal(traces[[14L]]$attributes$server.address, purl[["hostname"]])
    ...
    test_equal(traces[[15L]]$attributes$rpc.system, "nng")
    test_equal(traces[[15L]]$attributes$server.address, purl[["hostname"]])
    ...
    test_equal(traces[[15L]]$attributes$mirai.compute.profile, "default")
  }
}
test_false(daemons(0))
Sys.sleep(1L)
