...
connection && requireNamespace("otel", quietly = TRUE) && Sys.getenv("NOT_CRAN") == "true" && {
  ...
  length(record) && {
    ...
    test_equal(record$traces[[1L]]$status, "ok")
    ...
    test_equal(record$traces[[5L]]$status, "ok")
    ...
    test_equal(record$traces[[7L]]$status, "ok")
    ...
    test_equal(record$traces[[11L]]$description, "interrupt")
    test_equal(record$traces[[12L]]$name, "mirai::daemon")
    test_equal(record$traces[[12L]]$kind, "internal")
    test_equal(record$traces[[12L]]$status, "ok")
    ...
    test_equal(record$traces[[13L]]$status, "ok")
