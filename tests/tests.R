...
connection && requireNamespace("otel", quietly = TRUE) && Sys.getenv("NOT_CRAN") == "true" && {
  ...
  length(record) && {
    ...
    test_equal(traces[[1L]]$name, "mirai::daemons")
    ...
    test_equal(traces[[1L]]$events[[1L]]$name, "daemons->set")
    test_equal(traces[[2L]]$name, "mirai::mirai")
    ...
    test_equal(traces[[5L]]$name, "mirai::mirai_map")
    test_equal(traces[[5L]]$kind, "internal")
    test_equal(traces[[5L]]$links[[1L]]$span_id, traces[[5L]]$links[[1L]]$span_id)
    test_equal(traces[[5L]]$span_id, traces[[3L]]$parent)
    test_equal(traces[[5L]]$links[[1L]]$span_id, traces[[1L]]$span_id)
    test_equal(traces[[6L]]$name, "mirai::mirai")
    ...
    test_equal(traces[[8L]]$name, "mirai::daemon")
    test_equal(traces[[8L]]$kind, "internal")
    test_equal(traces[[8L]]$attributes$url, url)
    test_equal(traces[[8L]]$events[[1L]]$name, "daemon->start")
    test_equal(traces[[9L]]$name, "mirai::daemon->eval")
    ...
    test_equal(traces[[14L]]$name, "mirai::daemon")
    ...
    test_equal(traces[[14L]]$events[[1L]]$name, "daemon->end")
    test_equal(traces[[15L]]$name, "mirai::daemons")
    ...
    test_equal(traces[[15L]]$events[[1L]]$name, "daemons->reset")
  }
}
test_false(daemons(0))
Sys.sleep(1L)
