...
connection && requireNamespace("otel", quietly = TRUE) && Sys.getenv("NOT_CRAN") == "true" && {
  ...
  length(record) && {
    ...
    test_equal(length(traces), 14L)
    test_equal(traces[[1L]]$name, "mirai::mirai")
    test_equal(traces[[1L]]$kind, "client")
    test_true(traces[[2L]]$attributes$mirai.id > traces[[1L]]$attributes$mirai.id)
    test_equal(traces[[1L]]$parent, traces[[2L]]$parent)
    test_equal(traces[[2L]]$links[[1L]]$span_id, traces[[3L]]$links[[1L]]$span_id)
    test_equal(traces[[4L]]$name, "mirai::mirai_map")
    test_equal(traces[[4L]]$kind, "internal")
    test_equal(traces[[4L]]$links[[1L]]$span_id, traces[[3L]]$links[[1L]]$span_id)
    test_equal(traces[[4L]]$span_id, traces[[2L]]$parent)
    test_equal(traces[[5L]]$name, "mirai::mirai")
    test_equal(traces[[5L]]$kind, "client")
    test_equal(traces[[5L]]$parent, record$traces[[4L]]$parent)
    test_equal(traces[[7L]]$name, "mirai::daemon")
    test_equal(traces[[7L]]$kind, "internal")
    test_equal(traces[[7L]]$attributes$url, url)
    test_equal(traces[[7L]]$events[[1]]$name, "daemon->start")
    test_equal(traces[[8L]]$name, "mirai::daemon->eval")
    test_equal(traces[[8L]]$kind, "server")
    test_equal(traces[[8L]]$parent, traces[[1L]]$span_id)
    test_equal(traces[[8L]]$links[[1L]]$span_id, traces[[7L]]$span_id)
    test_equal(traces[[11L]]$status, "error")
    test_equal(traces[[11L]]$description, "miraiError")
    test_equal(traces[[12L]]$status, "error")
    test_equal(traces[[12L]]$description, "miraiInterrupt")
    test_equal(traces[[13L]]$name, "mirai::daemon")
    test_equal(traces[[13L]]$kind, "internal")
    test_equal(traces[[13L]]$attributes$url, url)
    test_equal(traces[[13L]]$links[[1L]]$span_id, traces[[7L]]$span_id)
    test_equal(traces[[13L]]$events[[1L]]$name, "daemon->end")
    test_equal(traces[[14L]]$name, "mirai::daemons")
    test_equal(traces[[14L]]$kind, "internal")
    test_equal(traces[[14L]]$attributes$url, url)
    test_equal(traces[[14L]]$attributes$n, 1L)
    test_equal(traces[[14L]]$attributes$dispatcher, "false")
    test_equal(traces[[14L]]$attributes$compute_profile, "default")
    test_equal(traces[[14L]]$span_id, traces[[1L]]$links[[1L]]$span_id)
    test_equal(traces[[14L]]$span_id, traces[[4L]]$links[[1L]]$span_id)
    test_equal(traces[[14L]]$span_id, traces[[5L]]$links[[1L]]$span_id)
  }
}
test_false(daemons(0))
Sys.sleep(1L)
