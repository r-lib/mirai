...
test_error(daemons(1, dispatcher = 1L), "should be TRUE, FALSE")
test_error(daemons(url = local_url(), dispatcher = 0), "should be TRUE, FALSE")
...
connection && Sys.getenv("NOT_CRAN") == "true" && {
  ...
  test_identical(status()$dispatcher, nextget("dispatcher"))
  with(daemons(url = status()$daemons, dispatcher = status()$dispatcher, .compute = "pool"), {
