...
#' variables.
#'
#' @param args (named list) configuration for HTTP daemon launching. By default,
#'   uses `posit_workbench_info()` to auto-configure from the environment. For
#'   custom configurations, must include the following named elements:
#'   - `url` (character) URL endpoint for the launch API (required).
#'   - `method` (character) HTTP method, typically `"POST"` (required).
#'   - `cookie` (character or function) session cookie value. May be a
#'     function returning the cookie value (optional, if required for
#'     authentication).
#'   - `token` (character or function) authentication bearer token. May be a
#'     function returning the token value. (optional, if required for
#'     authentication).
#'   - `data` (character) JSON or formatted request body containing the daemon
#'     launch command. Should include a placeholder `"%s"` where the host URL
#'     will be inserted at launch time (required).
#'
#' @inherit remote_config return
#'
...
#' @export
#'
http_config <- function(args = posit_workbench_info()) {
  c(list(type = "http"), args)
}
...
posit_workbench_info <- function() {
  ...
  nzchar(server) && nzchar(cookie) || stop(._[["posit_api"]])
  info <- ncurl(url = file.path(server, "api", "get_compute_envs"), headers = c(Cookie = cookie))
  info[["status"]] == 200L || stop(._[["posit_api"]])
  cluster <- secretbase::jsondec(info[["data"]])[["result"]][["clusters"]][[1L]]
  name <- cluster[["name"]]
  image <- cluster[["defaultImage"]]
  profile <- cluster[["resourceProfiles"]][[1L]][["name"]]
  lp <- sprintf(".libPaths(\"%s\")", .libPaths()[[1L]])
  cmd <- sprintf("{%s;%s}", lp, "mirai::daemon(\"%s\")")
  json <- list(
    method = "launch_job",
    kwparams = list(
      job = list(
        cluster = name,
        container = list(image = image),
