...
#'   \cr Alternatively this may be a character dispatcher URL if attaching to an
#'   existing dispatcher to share a common pool of daemons.
  ...
  if (is.character(url)) {
    if (is.null(envir)) {
      ...
      switch(
        ...
        {
          if (is.character(dispatcher) &&
              dial(sock, url = dispatcher, autostart = NA, fail = 3L) == 0L) {
            cv <- cv()
            if (is.null(serial)) serial <- .[["serial"]]
            if (is.list(serial)) `opt<-`(sock, "serial", serial)
            store_dispatcher(envir, sock, cv, dispatcher, url)
          } else {
...
#'     \item **dispatcher** (present only if using dispatcher) - character URL
#'     at which dispatcher is listening for hosts.
#'     \item **daemons** - character URL at which host / dispatcher is
#'     listening for daemons, or else `0L` if daemons have not yet been set.
...
parse_dispatcher <- function(x)
  if (is.logical(x)) 1L + (!is.na(x) && x) else if (is.character(x) && x == "none") 1L else 3L
...
store_dispatcher <- function(envir, sock, cv, urld, res) {
  ...
  `[[<-`(envir, "stream", NULL)
  `[[<-`(envir, "url", res[1L])
  length(res > 1L) && return(`[[<-`(envir, "pid", as.integer(res[2L])))
}
