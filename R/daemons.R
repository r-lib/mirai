...
req_socket <- function(listen = NULL, dial = NULL, tls = NULL, resend = 0L)
  `opt<-`(socket("req", listen = listen, dial = dial, tls = tls), "req:resend-time", resend)
...
store_dispatcher <- function(res, cv, envir) {
  `[[<-`(envir, "sock", res[[1L]])
  `[[<-`(envir, "urls", res[[2L]][-1L])
