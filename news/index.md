...
- Fixes a custom unserialization function (`ufunc`) that errors crashing
  the daemon; the object now unserializes as an ‘errorValue’ 1000
  instead (thanks [@eitsupi](https://github.com/eitsupi),
  [\#646](https://github.com/r-lib/mirai/issues/646)).
...
- [`http_config()`](https://mirai.r-lib.org/reference/http_config.md)
  ...
  key). `cookie` and `token` are retained as convenience arguments that
  append `Cookie:` and `Authorization: Bearer` entries to `headers`
  (thanks [@ddl-dkelkhoff](https://github.com/ddl-dkelkhoff),
  [\#612](https://github.com/r-lib/mirai/issues/612)).
- Reduces overhead for synchronous daemons by using an in-process
  transport.
...
- Adds `memory` argument to
  [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) setting a
  memory budget (MB, metric) for queued task payloads at dispatcher,
  providing memory-based backpressure. This is opt-in and `NULL`
  (default) is unbounded (thanks
...
- Adds [`try_mirai()`](https://mirai.r-lib.org/reference/mirai.md), a
  non-blocking variant of
  [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) that returns
  `NULL` immediately if the dispatcher’s `memory` budget is exhausted,
  instead of blocking. Useful in event-loop contexts (Shiny, promises)
  where blocking the host R thread is unacceptable.
...
- [`everywhere()`](https://mirai.r-lib.org/reference/everywhere.md) adds
  argument `.min` to specify a minimum number of daemons on which to
  evaluate the expression (when using dispatcher). This creates a
...
- Removes the following developer features:
  ...
  - Argument `id` is removed at
    [`daemon()`](https://mirai.r-lib.org/reference/daemon.md). This
    means that [`status()`](https://mirai.r-lib.org/reference/status.md)
    no longer reports daemon connection or disconnection events.
- Removes deprecated argument `tls` at
  [`daemon()`](https://mirai.r-lib.org/reference/daemon.md),
...
- Fixes a phenomenon where
  ...
  `mirai_map()[.stop]` could on occasion cause (dispatcher) daemons to
  be interrupted and exit on subsequent runs
...
- Adds [`info()`](https://mirai.r-lib.org/reference/info.md) as an
  alternative to
  [`status()`](https://mirai.r-lib.org/reference/status.md) for
...
- Adds
  ...
  [`with.miraiDaemons()`](https://mirai.r-lib.org/reference/with.miraiDaemons.md)
...
- [`stop_mirai()`](https://mirai.r-lib.org/reference/stop_mirai.md) is
  more efficient and responsive, especially for ‘mirai_map’ objects
...
- [`mirai_map()`](https://mirai.r-lib.org/reference/mirai_map.md) now
  supports Arrow Tables and Polars DataFrames
...
- A [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) evaluated on
  an ephemeral daemon returns invisibly, consistent with other cases
  ([\#351](https://github.com/r-lib/mirai/issues/351)).
- [`daemon()`](https://mirai.r-lib.org/reference/daemon.md) gains a
  `tlscert` argument for custom TLS certificates. The change in argument
  name lets this be passed when making a
  [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) call
...
- [`everywhere()`](https://mirai.r-lib.org/reference/everywhere.md) has
  been updated for robustness and ease of use:
...
- An ephemeral daemon started by
  [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) without
  setting daemons now exits as soon as the parent process does rather
  than finish the task.
- Change in [`daemon()`](https://mirai.r-lib.org/reference/daemon.md)
  defaults:
...
- Calling [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) to
  create local daemons now errors if performed within a
  [`mirai_map()`](https://mirai.r-lib.org/reference/mirai_map.md) call.
  This guards against excessive spawning of local processes on a single
  machine.
...
- [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) argument
...
- [`host_url()`](https://mirai.r-lib.org/reference/host_url.md) is
  ...
  using a hostname.
- Adds
  [`register_serial()`](https://mirai.r-lib.org/reference/register_serial.md)
  to register serialization configurations for all
  [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) calls (may
  be used by package authors as a convenience).
- Adds [`on_daemon()`](https://mirai.r-lib.org/reference/on_daemon.md)
...
- [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) now
...
- Fixes
  ...
  [`daemon()`](https://mirai.r-lib.org/reference/daemon.md) (thanks
  [@LennardLux](https://github.com/LennardLux),
  [\#240](https://github.com/r-lib/mirai/issues/240)).
- [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) dispatcher
  argument ‘process’, deprecated in mirai v2.1.0, is removed.
...
- Simplified SSH tunnelling for distributed computing:
  ...
  - [`local_url()`](https://mirai.r-lib.org/reference/host_url.md) adds
    logical argument ‘tcp’ for easily constructing an automatic local
    TCP URL when setting
    [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) for SSH
    tunnelling.
...
- mirai (in R \>= 4.5) is now one of the official base R parallel
  ...
  - Directly use `parallel::makeCluster(type = "MIRAI")` to create a
    ‘miraiCluster’.
- [`call_mirai()`](https://mirai.r-lib.org/reference/call_mirai.md) is
  now user-interruptible, consistent with all other functions in the
  package.
...
- [`with()`](https://rdrr.io/r/base/with.html) method for
  ...
  functions such as
  [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) within the
  [`with()`](https://rdrr.io/r/base/with.html) clause.
- [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) arguments
...
- [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) calls that
  error due to the remote launcher no longer leave the compute profile
  set up ([\#237](https://github.com/r-lib/mirai/issues/237)).
...
- [`mirai_map()`](https://mirai.r-lib.org/reference/mirai_map.md) now
  errors if daemons have not yet been set (rather than warn and launch
  one local daemon).
...
- [`daemon()`](https://mirai.r-lib.org/reference/daemon.md) now returns
  an integer exit code to indicate the reason for termination.
- Adds [`nextcode()`](https://mirai.r-lib.org/reference/nextstream.md)
  to provide a human-readable translation of the exit codes returned by
  [`daemon()`](https://mirai.r-lib.org/reference/daemon.md).
- [`everywhere()`](https://mirai.r-lib.org/reference/everywhere.md) now
  returns a list of at least one mirai regardless of the number of
  actual connections.
...
- [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) gains
...
- [`stop_mirai()`](https://mirai.r-lib.org/reference/stop_mirai.md) is
  now able to cancel remote mirai tasks (when using dispatcher),
...
- [`status()`](https://mirai.r-lib.org/reference/status.md) using the
  new dispatcher is updated to provide more concise and insightful
  information.
- [`everywhere()`](https://mirai.r-lib.org/reference/everywhere.md)
  updates:
  ...
  - Drops argument ‘.serial’ as serialization configurations are now
    ...
    [`daemons()`](https://mirai.r-lib.org/reference/daemons.md).
- [`daemon()`](https://mirai.r-lib.org/reference/daemon.md) updates:
  ...
  - Gains argument ‘id’ which accepts an integer value that allows
    [`status()`](https://mirai.r-lib.org/reference/status.md) to track
...
- [`host_url()`](https://mirai.r-lib.org/reference/host_url.md) argument
  ‘ws’ is removed as a TCP URL is now always recommended (although
  websocket URLs are still supported).
...
- [`mirai_map()`](https://mirai.r-lib.org/reference/mirai_map.md) adds
  `[.progress_cli]` as an alternative progress indicator, using the cli
  package to show % complete and ETA.
- [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) gains
  argument ‘force’ to control whether further calls reset previous
  settings for the same compute profile.
- [`daemon()`](https://mirai.r-lib.org/reference/daemon.md) gains
  argument ‘asyncdial’ to allow control of connection behaviour
  independently of what happens when the daemon exits.

##### Behavioural Changes

- For [`daemons()`](https://mirai.r-lib.org/reference/daemons.md):
...
- For [`mirai_map()`](https://mirai.r-lib.org/reference/mirai_map.md):
...
- `dispatcher()` drops argument ‘asyncdial’ as it is rarely useful to
  set this here.
- [`everywhere()`](https://mirai.r-lib.org/reference/everywhere.md) now
  errors if the specified compute profile is not yet set up, rather than
  fail silently.
- [`launch_local()`](https://mirai.r-lib.org/reference/launch_local.md)
  and
  [`launch_remote()`](https://mirai.r-lib.org/reference/launch_local.md)
...
- Fixes [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) to
  correctly handle a vector of URLs passed to ‘url’ again.
- Fixes flatmap with `mirai_map()[.flat]` assigning a variable ‘typ’ to
  the calling environment.
- Performance enhancements for
  [`mirai()`](https://mirai.r-lib.org/reference/mirai.md),
  [`mirai_map()`](https://mirai.r-lib.org/reference/mirai_map.md) and
  the promises method.
...
- [`everywhere()`](https://mirai.r-lib.org/reference/everywhere.md) adds
  argument ‘.serial’ to accept serialization configurations created by
  [`serial_config()`](https://mirai.r-lib.org/reference/serial_config.md).
...
- `serialization()` is now deprecated in favour of the above usage of
  [`everywhere()`](https://mirai.r-lib.org/reference/everywhere.md), and
...
- Faster local `daemons(dispatcher = FALSE)` and
  [`make_cluster()`](https://mirai.r-lib.org/reference/make_cluster.md)
...
- Fixes rare cases of
  [`everywhere()`](https://mirai.r-lib.org/reference/everywhere.md) not
  reaching all daemons when using dispatcher.
...
- Adds [`mirai_map()`](https://mirai.r-lib.org/reference/mirai_map.md)
...
- [`unresolved()`](https://mirai.r-lib.org/reference/unresolved.md),
  ...
  [`collect_mirai()`](https://mirai.r-lib.org/reference/collect_mirai.md)
  and [`stop_mirai()`](https://mirai.r-lib.org/reference/stop_mirai.md)
  now accept a list of ‘mirai’ such as that returned by
  [`mirai_map()`](https://mirai.r-lib.org/reference/mirai_map.md).
- Improved mirai print method indicates whether a mirai has resolved.
- Calling [`daemons()`](https://mirai.r-lib.org/reference/daemons.md)
  with new settings when the compute profile is already set now
  implicitly resets daemons before applying the new settings instead of
  silently doing nothing.
- Argument ‘resilience’ retired at
  [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) as
...
- Behavioural changes to
  [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) and
  [`everywhere()`](https://mirai.r-lib.org/reference/everywhere.md):
...
- [`stop_mirai()`](https://mirai.r-lib.org/reference/stop_mirai.md) now
  resolves to an ‘errorValue’ 20 (operation canceled) in the case the
  asynchronous task was still ongoing (thanks
...
- Fixes [`daemons()`](https://mirai.r-lib.org/reference/daemons.md)
...
- Adds [`local_url()`](https://mirai.r-lib.org/reference/host_url.md)
...
- [`daemon()`](https://mirai.r-lib.org/reference/daemon.md) argument
  ‘autoexit’ now accepts a signal value such as
  [`tools::SIGINT`](https://rdrr.io/r/tools/pskill.html) in order to
  raise it upon exit.
- [`daemon()`](https://mirai.r-lib.org/reference/daemon.md) now records
  the state of initial global environment objects (e.g. those created in
  .Rprofile) for cleanup purposes (thanks
...
- Simplification of
  [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) interface:
  ...
  - ‘.signal’ argument removed - now all ‘mirai’ signal if daemons are
    set up.
- [`everywhere()`](https://mirai.r-lib.org/reference/everywhere.md) now
  returns invisible NULL in the case the specified compute profile is
  not set up, rather than error.
- [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) specifying a
  timeout when
  [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) has not
  been set - the timeout begins immediately rather than after the
...
- Fixes [`mirai()`](https://mirai.r-lib.org/reference/mirai.md)
...
- [`host_url()`](https://mirai.r-lib.org/reference/host_url.md)
  implemented as a helper function to automatically construct the host
  URL using the computer’s hostname.
- [`daemon()`](https://mirai.r-lib.org/reference/daemon.md) adds
...
- Implements an alternative communications backend for R, adding methods
  ...
  - [`status()`](https://mirai.r-lib.org/reference/status.md) can now
    take a ‘miraiCluster’ as the argument to query its connection
    status.
- [`launch_remote()`](https://mirai.r-lib.org/reference/launch_local.md)
  improvements:
...
- [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) gains the
  following features:
...
- [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) adds logical
  argument ‘.signal’ for whether to signal the condition variable within
  the compute profile upon resolution of the ‘mirai’.
- [`daemon()`](https://mirai.r-lib.org/reference/daemon.md) argument
...
- Eliminates a potential crash in the host process after querying
  [`status()`](https://mirai.r-lib.org/reference/status.md) if there is
  no longer a connection to dispatcher.
- Reverts the trailing line break added to the end of a ‘miraiError’
  character string.
- Moves the ‘…’ argument to
  [`daemons()`](https://mirai.r-lib.org/reference/daemons.md),
  `dispatcher()` and
  [`daemon()`](https://mirai.r-lib.org/reference/daemon.md) to clearly
  delineate core vs peripheral options.
...
- Deprecated use of alias `server()` for
  [`daemon()`](https://mirai.r-lib.org/reference/daemon.md) is retired.
...
- Uses L’Ecuyer-CMRG streams for safe and reproducible (in certain
  ...
  Project Sprint 2023).
  - [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) gains
    the new argument ‘seed’ to set a random seed for generating these
    streams.
  - [`daemon()`](https://mirai.r-lib.org/reference/daemon.md) and
    `dispatcher()` gain the argument ‘rs’ which takes a L’Ecuyer-CMRG
    random seed.
- New developer functions
  [`nextstream()`](https://mirai.r-lib.org/reference/nextstream.md) and
  [`nextget()`](https://mirai.r-lib.org/reference/nextstream.md),
...
- Tweaks to [`status()`](https://mirai.r-lib.org/reference/status.md):
...
- Secure TLS connections implemented for distributed computing:
  ...
  - Alternatively, custom certificates may be passed to the ‘tls’
    argument of
    [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) and
    [`daemon()`](https://mirai.r-lib.org/reference/daemon.md), such as
    those generated via a Ceritficate Signing Request (CSR) to a
...
- User interface optimised for consistency and ease of use:
  ...
  - [`daemon()`](https://mirai.r-lib.org/reference/daemon.md) replaces
    `server()`, which is deprecated (although currently retained as an
    alias).
  - [`launch_local()`](https://mirai.r-lib.org/reference/launch_local.md)
  ...
  - [`status()`](https://mirai.r-lib.org/reference/status.md) now
    retrieves connections and daemons status, replacing the call to
    [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) with no
    arguments (which is deprecated). The return value of `$daemons` is
    now always the host URL when not using dispatcher.
- Redirection of stdout and stderr from local daemons to the host
  ...
  [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) or
  [`launch_local()`](https://mirai.r-lib.org/reference/launch_local.md).
  [`daemon()`](https://mirai.r-lib.org/reference/daemon.md) accepts a
  new ‘output’ argument.
...
- Improvements to dispatcher:
  ...
  - [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) status
    requests have been rendered more robust.
...
- `server()` and `dispatcher()` argument ‘asyncdial’ is now FALSE by
  ...
  purposes, [`daemons()`](https://mirai.r-lib.org/reference/daemons.md)
  should be called before `server()` is launched on remote resources, or
  else `server(asyncdial = TRUE)` allows servers to wait for a
  connection.
...
- The deferred evaluation pipe `%>>%` gains the following enhancements:
  ...
  - expressions may be tested using
    [`unresolved()`](https://mirai.r-lib.org/reference/unresolved.md) in
    the same way as a ‘mirai’.
...
- [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) gains the
...
- Fixes potential issue when attempting to use
  [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) with timeouts
  and no connection to a server.
...
- Compute profiles have been introduced through a new `.compute`
  argument in
  [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) and
  [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) for sending
  tasks with heterogeneous compute requirements.
  - [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) can
    create new profiles to connect to different resources e.g. servers
    with GPU, accelerators etc.
  - [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) tasks can be
    sent using a specific compute profile.
- [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) interface
  has a new `url` argument along with `dispatcher` for using a
...
- [`daemons()`](https://mirai.r-lib.org/reference/daemons.md) now takes
  ‘n’ and ‘.url’ arguments. ‘.url’ is an optional client URL allowing
  mirai tasks to be distributed across the network. Compatibility with
...
- [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) gains a
...
- Adds ‘.timeout’ argument to
  [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) to ensure a
  mirai always resolves even if the child process crashes etc.
...
- Back to a pure R implementation thanks to enhanced internal design at
  nanonext.
- Adds auxiliary function
  [`is_mirai()`](https://mirai.r-lib.org/reference/is_mirai.md) to test
  if an object is a mirai.
...
- [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) added as an
  alias for `eval_mirai()`; supports evaluating arbitrary length
...
- Auxiliary functions
  [`is_nul_byte()`](https://nanonext.r-lib.org/reference/is_error_value.html)
  and [`unresolved()`](https://mirai.r-lib.org/reference/unresolved.md)
  re-exported from {nanonext} to test for evaluation errors and
  resolution of a ‘mirai’ respectively.
- New [`daemons()`](https://mirai.r-lib.org/reference/daemons.md)
