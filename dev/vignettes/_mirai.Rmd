...
`daemons()` has a `with()` method, which evaluates an expression with daemons created for the duration of the expression and automatically torn down upon completion. All mirai calls within the `with()` clause default to using the compute profile of the daemons created.
