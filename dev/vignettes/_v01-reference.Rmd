...
For CPU-bound work, set `n` to roughly `ps::ps_cpu_count() - 1`, leaving a core free for the host R process and OS, and accounting for any cores reserved for other purposes.
...
`capacity` reflects the value set above (`NA` if unset).
Use `peak` to size `memory`: profile a representative workload with `memory = NULL` first to capture organic demand, then set the capacity at or above the observed peak.
...
`share()` and `memory =` compose: a `share()`-wrapped argument shrinks the queued payload to ~30 bytes, sidestepping backpressure on the input side.
