...
A `mirai_map()` call returns almost immediately.
...
> Use of `mirai_map()` requires that `daemons()` have previously been set, and will error if this is not the case. This avoids launching potentially too many *ephemeral daemons* if the map is over a large number of elements.
...
1. The '.promise' argument allows a promise action to be registered against each iteration. This can be used to perform side-effects when each iteration completes (such as checkpointing or recording a progress update).
...
```
Use `...` to further specify objects referenced but not defined in `.f` - the function `fn` below:
```{r}
#| label: mmap2
