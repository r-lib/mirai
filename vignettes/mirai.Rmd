...
Futures are an abstraction that represent the result of code evaluation that will be available at some point in the future.

The actual code evaluation is sent to and performed in a separate R process (*daemon*), and the result is sent back to the main (*host*) process when it completes.
This means that a `mirai()` call that creates a mirai returns almost immediately, and is never blocking. This is the essence of async: whilst the mirai evaluation is ongoing on the daemon, the host R process is free to continue.
...
A mirai is either *unresolved* if the result has yet to be received, or *resolved* if it has. Use `unresolved()` to check for the state of a mirai.

When a mirai has resolved, the result is available at `$data`.
If successful, this will be the return value of the evaluated expression.
If the expression threw an error or caused the process to crash then this will be an 'errorValue' instead.
See the section [Errors in a mirai](#errors-in-a-mirai) below.

Instead of repeatedly checking `unresolved()`, to wait for a mirai to resolve and then return its `$data` value, use it's `[]` method, or equivalently `collect_mirai()`.
...
Instead, it is for the end-user running the code to declare the resources available using a `daemons()` call. `daemons_set()` may be used to check if these have been set up for a particular session.

If daemons are not set, each `mirai()` call will by default create a new local process (*ephemeral daemon*) on which to perform its evaluation.

Instead `daemons()` sets up persistent daemons on which to evaluate mirai expressions.

- Using persistent daemons eliminates the time and overhead of starting new processes for each evaluation, and allows control over the number of processes used at any one time.
...
collect_mirai(m)
#> [1] 3.550325 4.770648 4.121557 5.044731 3.792272
m[]
#> [1] 3.550325 4.770648 4.121557 5.044731 3.792272
```
...
m[]
#> [1] 2.888348 4.275282 2.945653 4.850117 4.306285
```
...
#> 
#> $daemons
#> [1] "ipc:///tmp/e8b1d23e26751cc5e0db7231"
#> 
#> $mirai
...
#> 
#> $daemons
#> [1] "ipc:///tmp/f53b80cc9e4f3e14d759ba02"
```
...
launch_remote()
#> [1]
#> Rscript -e 'mirai::daemon("tcp://192.168.5.57:65269")'
daemons(0)
#> [1] 0
```
...
launch_remote(1)
#> [1]
#> Rscript -e 'mirai::daemon("tls+tcp://192.168.5.57:65274",tls=c("-----BEGIN CERTIFICATE-----
#> MIIFPzCCAyegAwIBAgIBATANBgkqhkiG9w0BAQsFADA3MRUwEwYDVQQDDAwxOTIu
#> MTY4LjUuNTcxETAPBgNVBAoMCE5hbm9uZXh0MQswCQYDVQQGEwJKUDAeFw0wMTAx
#> MDEwMDAwMDBaFw0zMDEyMzEyMzU5NTlaMDcxFTATBgNVBAMMDDE5Mi4xNjguNS41
#> NzERMA8GA1UECgwITmFub25leHQxCzAJBgNVBAYTAkpQMIICIjANBgkqhkiG9w0B
#> AQEFAAOCAg8AMIICCgKCAgEAoGraApc7r1LRWlOLwY8eZmNG66TGxGxzi/wvnY2Q
#> 8pYM4OhtDzrTG2ISQxVu6Np3gRZMOV6pJCGnU2FpYRJyQ5Z+0JYrUzW1lzsTBijA
#> gwFxVcWA2mb0tOrK4VdWA03ErmCiikG0g5voaqmmAL7M7h/wkhCcTpNiNOprHwsA
#> wbfE58Vc5W4jGSV2hBZRgVGNXZ2m4ICUqnFSUjdX2JQN7iEFzW/iYw9lcwo8U1FK
#> PvpXzNgNdu2uzVgl/8z1uouP9lZLURR1t8hfdbtvktKt874/jd0DeV/LEpLaa58i
#> KJv9nNp3iNsNlR7oE3+Q2GJDtF6GlSs/T3K/RDnWKSUdxKmt+yRvIti19dgSvdhZ
#> /z2mpmY5dFtmFfrP+1xcVIWE8QV+X+mloNO9JgfUj3HvS052UsmbfbOJVQF4yqXy
#> WC1I599XFzF43zRmoPrMllzzNZ5pcZ6fb1TqWmvAjD6CRkctlnDemErHfrbApMv9
#> X3aGyRFvlMCrmH/XZegfXDV8yHgBII879eYH+sowf5LEnKDD4qTVJLKTyYqwDotx
#> lXa1kILhG6dGFOVqpzbR/bSiuQxaa60dCQiFoXYj5XBApTPxD+3X2OiQkzijvpSR
#> maT0fGE8IVBWzXCyHXJLktilcj3/+zd7FyB6ZWDLWVKlE8rGJorhNH6e7rwLH1B9
#> N1UCAwEAAaNWMFQwEgYDVR0TAQH/BAgwBgEB/wIBADAdBgNVHQ4EFgQUJ8S73/oV
#> 0eP+HiSC6CVPgD6vEekwHwYDVR0jBBgwFoAUJ8S73/oV0eP+HiSC6CVPgD6vEekw
#> DQYJKoZIhvcNAQELBQADggIBAFln/0tO0YKumUwHff3FQGNaNR6cVjAdi++sgeXG
#> ocZNV/02PJf3/F2n3o/yAvmjvN05a5W1FokaOInnLCiTmChxuxxLGYtf0IARnBWL
#> pqLPrt2NczVk5pdQXsyo6chz5RRDeMm8PujNWFTZmxRo9uIxmV96RicLVnOd1Uhn
#> TZHUObM55p3eobz4yXFK+6PY8lFYqjFNswOFRiSY3iVyDEcOTsqi4YNdoUBWoePi
#> h0ScfQ/aasuzIjg/+vC84yJj2jYVXAESHTBi9kxlx7rIsWkAIh8T+m3Lj+v6uLK3
#> +wlgMQn5M6a7uZE4kJPZTeh6NYiX5ZHe6zJmtO1t0oiYVXfzhJ/rhLSspjX/fbxu
#> wWkBMKbygEuq+wftdKJ8JYDECBCE0JARvGf8moe6etAZg9sgoxZ84BgQw5n6zb/C
#> Xxr+8iImjxJQkxF7Tk6mamW5zawoibZ2B6HsjJxcCT99FNvi9+ZCFARcW0ZlhP/+
#> PbEVAakmTLvoaE+shIUOzbWxRDvyK3CEeIDfzSodvANX/XQ+2hr0HLkBDsfzTofc
#> 0yaWCEhd7fJDIEnVnjesQSEqkw2WxsSRyX2/s0hjM1/9YNx4SJB8aTEmkC4qV3Y5
#> Yt35I4/jHNVC8rzo1iDXRI5KT5WsS4chKaa0jWAZjGh2+Fd+vLy6AiwM8XbeNq7X
#> jJQm
#> -----END CERTIFICATE-----
#> ",""))'
```
