...
This is a cheatsheet. Refer to the [mirai reference manual](v00-mirai.html) for a detailed introduction.
...
## 13. Integration with R Parallel
...
# Create MIRAI cluster
cl <- make_cluster(4)
cl <- makeCluster(4, type = "MIRAI") # Available R >= 4.5

# Use with parallel functions
results <- parLapply(cl, 1:100, sqrt)
results <- mclapply(1:100, sqrt)

# Stop cluster
stopCluster(cl)
```

## 14. Integration with Promises

### Convert mirai to Promise

```r
library(promises)

# Promise then (implicit conversion)
mirai({Sys.sleep(1); "result"}) |> then(print)

# Explicit conversion
p <- as.promise(mirai(slow_task()))
p$then(onFulfilled = function(x) cat("Done:", x))
```
...
# In server function
task <- ExtendedTask$new(function(x) mirai(compute(x), x = x))

observeEvent(input$button, task$invoke(input$value))
observeEvent(task$result(), {
  output$plot <- renderPlot(task$result())
})
```

## 15. Dispatcher vs. Direct
