...
4. When launching daemons, the certificate chain to the CA should be supplied to the 'tlscert' argument of `launch_remote()` or `daemons()` to pass through to `daemon()`.
  ...
  - If these are concatenated together as a single character string `certchain`, then the character vector comprising this and an empty character string `c(certchain, "")` may be supplied to the relevant 'tlscert' argument.
