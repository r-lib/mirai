...
**`mirai::daemons`** - Root span for a compute profile

- **Kind**: `internal`
- **Attributes**: `url`, `n` (number of daemons), `dispatcher` (true/false), `compute_profile`
- **Events**: `daemons.set` or `daemons.reset`
- **Lifecycle**: Created when `daemons()` is called, and again when daemons are reset

**`mirai::daemon`** - Individual daemon process span

- **Kind**: `internal` 
- **Attributes**: `url`
- **Events**: `daemon.start` or `daemon.end`
- **Lifecycle**: Created when a daemon process starts and again when it ends
...
mirai::mirai_map (top level) ──link→ mirai::daemons
...
**Span Links**: Tasks are linked to their compute profile's `mirai::daemons` span on the client side, and to each `mirai::daemon` span on the server side, showing exactly where each evaluation happened. When `mirai::daemons` and `mirai::daemon` end, these events are recorded in new spans with links back to their original start spans.
