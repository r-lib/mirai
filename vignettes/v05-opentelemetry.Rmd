...
**`daemon` / `daemon exit`** - Individual daemon process span
...
- **Attributes**:
  ...
  + `server.port` (where applicable)
  + `network.transport` (e.g. 'tcp' or 'ipc')
- **Lifecycle**: Created when a daemon process starts, and when it exits
...
daemon exit (daemon process 1) ──link→ daemon
...
daemon exit (daemon process N) ──link→ daemon
