...
The package has one main function: `mirai()` to create a mirai object from an expression.

This function returns immediately, and is never blocking. This is the essence of async: whilst the mirai expression is evaluated on a daemon, the host process is free to move on.

As the expression is evaluated in another process, it must be self-contained:

- Package functions must be namespaced using `::`, or `library()` calls be made within the expression.
- Other functions/data/objects required by the expression should be passed explicitly via `...` or `.args`.

> This is a 100% reliable evaluation model that corresponds to the underlying reality - any attempt to abstract away from it is inherently imperfect, and we do not compromise on this point.
...
m[]
#> [1] 5.148091 5.955047 4.076417 4.922289 5.492950
m$data
#> [1] 5.148091 5.955047 4.076417 4.922289 5.492950
```
...
m1[]
#> [1] 2.430959 3.526444 5.823612 5.234169 3.958373
```

The example below uses `mirai()` to perform a long-running write operation asynchronously from a separate process.
Here `environment()`, which is the calling environment, is passed directly to '.args'.
This is a convenient method for passing in objects existing in the environment, such as the `x` and `file` provided to the `write.csv.async()` function.
...
When writing a `mirai()` call, you should not be concerned about where or how execution of that code actually happens.

It is for the end-user running the code to declare the resources available for evaluating mirai calls. This is done using the package's other main function: `daemons()`.
...
- The original condition classes are also preserved at `$condition.class`
...
```
- The elements of the original error condition are also accessible via `$` on the error object
- Additional metadata recorded by `rlang::abort()` is preserved
...
**By default, when the `seed` argument to `daemons()` is `NULL`**: mirai initiates a new stream for each daemon launched, in the same manner as base R.
...
**Supply an explicit integer `seed` to `daemons()` for reproducible RNG**: instead of initiating a new stream for each daemon, now a stream is initiated for each `mirai()` call.

- Guarantees the same deterministic, reproducible results across runs
- This is regardless of the number of daemons used
- This does require additional computation, although with a negligible impact on performance
...
To view current statistics, `info()` provides these succinctly in a single integer vector:
...
- `executing` number of mirai tasks currently being evaluated on a daemon
- `completed` number of mirai tasks for which have been either completed or cancelled
...
The `status()` function provides a slightly more verbose output:
...
#> 
#> $daemons
#> [1] "ipc:///tmp/1f05eb87154e3e23b90180ee"
#> 
#> $mirai
...
#> 
#> $daemons
#> [1] "ipc:///tmp/36704399135aaca5e1c2387e"
```
...
launch_remote()
#> [1]
#> Rscript -e 'mirai::daemon("tcp://192.168.1.110:53582")'
daemons(0)
```
...
launch_remote(1)
#> [1]
#> Rscript -e 'mirai::daemon("tls+tcp://192.168.1.110:53583",tlscert=c("-----BEGIN CERTIFICATE-----
...
#> DQEBAQUAA4ICDwAwggIKAoICAQCq25Iko/UYA05Y+fegKz/IdlDJgjenBbDN7B8s
#> No6b7j71sWoKUEJG0hoKEI3Jvu3Hk5DwSTIMZ7mVB/tFcFbHZXah7QiiudRh+XlV
#> e3K5sbac4CGg9C1Zy8w25YtI5X9XjVn+ZN+Tk65y0mqe09rXICSQf/BLgGz6daI4
#> sVY9cZlzCn9gBoeysxdjT+xOQAhk+zyFU3e7KZl1pPXcJ4c26i1ZNBTRtFCIUyB6
#> wZ0NJ/LTWWxLr4vzqq4vM+qdK+MlxF9cSMuJfWTrRnIobHrIQIGLCTQZhkhi/RWK
#> XPNPnFOPry7vtWoZCDZv5HGlqC494VXWgy4/7P1lXd/BqhOxySjBVtR2p3YpKNhY
#> Y9MRrQzm2/d8+9SvaDjeLI5yxTTdnegvvAx0ShGaTHwetIytnToiFpKreZ6QidgL
#> 2m3XV5rrfRYE1OGggOTiGkzG68hw28+n1Rvk+TFSev9fD37wbXY4VAdqgeKYKp9X
#> ebVP8Fn+i/pq4GD2MWQatLL68FIyAoSoUzJxJ9VNyPkdjHeCjmHJ4vPV5CgAsr/I
#> IGW2TL/onOIJfv4S0lPStSzE0T8gq8rsIf76JUPCcxQeguljQW48Gt48nMJsSyhk
#> c9Fo4AR9Qkl3NjS0FYQWbBbYWusyUtsjSZapAC5KuzE881puZLGOSCHGol1oYP9z
#> VPEz1QIDAQABo1YwVDASBgNVHRMBAf8ECDAGAQH/AgEAMB0GA1UdDgQWBBQjduKy
#> xJlN+zvm+U058lYZ/Wb6QTAfBgNVHSMEGDAWgBQjduKyxJlN+zvm+U058lYZ/Wb6
#> QTANBgkqhkiG9w0BAQsFAAOCAgEAqiBfUiBu1bjgy2IzGaxeFLr6j9dox6c84Ljx
#> aqUwchhK9ZHS6ZMXTpEjiMK3l7Y6NPrlRc+hOD4f124fho52FgLToGx23xhmTNw4
#> UVKTaeKF2idwtifAMHWkMJZk3k5ami5FOmsw/5fRClpCjMSEFnJWHXnSoHg1wPZ2
#> 0WZC61yx1Ni1Rt5jYekt2LeqL47yaxKjn8qNw+zT49IRuKHDH74kIlFLN6ldoTZZ
#> /f2G5iUvy5RJOokZXfyh/PtVRanfEw0csiOGIDj8U93c/tKXmSLDca7pqHC53eyD
#> tTGo32Ipx++7RpGBHdZmovJ6Zis/zqm3JBPc9G1h3sSHcE71oQurAXfxo57UXPIw
#> 5JrOklB1toY9sLQXnL88UqCGVLsOsY2DgmPMftggqMirQAqBS8zunyqoTCxpSrLC
#> GCZ+1BmPrNRcwY8vTpvMlmyNjk7p4dOyxl8kXaWnjeuiUUAsHea22LiMseojluls
#> VNM4QV4Nhr/08NpJBOuNGJIsvmml8xfTDtRKSREHbTZ8csAz4HvGDaxNTWvUMuC3
#> C1N+9h8RmqT4l4RhZ+ocESj0/u/vc+TMmX8sM0MGS4sJ0pHjypy/z1FD578VHOkl
#> NnKidFFxkgWPpjtuzX7JLktiktF86pSAetP/qktIVSx8IZOORTwZrCkgRq5wukca
#> iVi7HBA=
#> -----END CERTIFICATE-----
#> ",""))'
```
...
s1$daemons
#> [1] "ipc:///tmp/3068d5fe2cf5c2509e152553"
m1[]
#> [1] 47956

s2$daemons
#> [1] "ipc:///tmp/4cad08e5df6ce1f84dc02821"
m2[] # different to m1
#> [1] 47982
...
`daemons(sync = TRUE)` turns on synchronous mode for the default compute profile. This causes mirai to be evaluated immediately after creation, and before they return, with no asynchronous operation. This can be useful for testing and debugging purposes. In particular, it allows you to drop into an interactive browser session from a `browser()` statement inside your mirai expression.

To restrict synchronous behaviour to a specific compute profile, additionally specify `.compute`. A value of `seed` may be supplied for reproducible parallel RNG, but all other arguments have no effect when specifying `sync = TRUE`.
...
`mirai_map()` performs asynchronous parallel/distributed map over a list or vector, returning a 'mirai_map' object.

> Requires `daemons()` to be set first. This avoids launching potentially too many ephemeral daemons.

#### Basic Usage

Returns immediately with evaluations running asynchronously. Collect results using `x[]`.
...
Dataframes and matrices map over **rows**. Function `.f` must accept as many arguments as there are columns.
...
For matrices, rows are passed to `.f`:
...
For nested mapping, launching local daemons from within `mirai_map()` will error. Instead use:
```r
