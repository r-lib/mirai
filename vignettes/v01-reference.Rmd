...
m[]
#> [1] 6.733118 5.918639 6.495747 2.926507 4.707139
m$data
#> [1] 6.733118 5.918639 6.495747 2.926507 4.707139
```
...
m1[]
#> [1] 3.228269 4.489396 2.776603 4.001947 5.835455
```
...
The default `dispatcher = TRUE` enables optimal FIFO scheduling.
...
By default, the dispatcher queues incoming tasks without bound.
This risks host out-of-memory when tasks arrive faster than daemons consume them, particularly with large payloads.
...
`capacity` reflects the budget set above (`NA` if unset).
Use `peak` to size the `memory` capacity: run a representative workload and set the budget at or above the observed value.

#### Non-blocking Submission

Blocking the host R thread is acceptable in batch scripts, but not in event-loop contexts such as Shiny or promises.

`try_mirai()` is a non-blocking variant of `mirai()`.
It returns `NULL` invisibly when the queue is at the memory limit, instead of blocking.
The caller decides what to do — drop the task, retry later, or raise a condition.
...
With `memory` unset or no dispatcher, `try_mirai()` always returns a mirai and behaves identically to `mirai()`.
...
Only the ~30-byte shared memory name is serialised across the wire; the daemon accesses the data directly via ALTREP.
`lobstr::obj_size(y)` on the daemon reflects just the wrapper, confirming the 80 MB vector was never copied across.
...
launch_remote()
#> [1]
#> Rscript -e 'mirai::daemon("tcp://192.168.7.113:53516")'
```
...
launch_remote()
#> [1]
#> Rscript -e 'mirai::daemon("tcp://192.168.7.113:53517")'
daemons(0)
```
...
launch_remote(1)
#> [1]
#> Rscript -e 'mirai::daemon("tls+tcp://192.168.7.113:53518",tlscert=c("-----BEGIN CERTIFICATE-----
...
#> DQEBAQUAA4ICDwAwggIKAoICAQCqqJ6DzpRZDqixv0wr42maHSrcv1j03QfYjCHj
#> FKHz8FP+3boWE2WePRY6skJVzpz4u3SgGEHC44cRfOs5c9M0XNeCUjczYSZ2epp+
#> mvX06n58083+EbQbxUm7JHZiBwthw5tY5VB8w7AI1YiiJ68xpaYAq88p5Eb90ZpP
#> maDwhCMwr9podxjYPGTmtl5bUNsQjnyecCOf7TI2ZlbyYrohbgKpZazvcni1bBIw
#> OpgkDmX+HJYcvk8imTL5y0BCvHUpQZecJg2yn2XGUFerNex7RCgxT2EKn7dyU/ka
#> 0nySn7rvvCulTo+XCnTm6oleOI1nSK/+5XugOJ2IDqDegyYA2JOEfPIhidCDgjed
#> +Ufs0iNI0LUigjQ1ZZifeJWFzn3libhdQd+ybDwkoW+Gp8srqlCYU/z8n9iSTdPZ
#> w4h+ViY2n3RD/1ghiCuAGNfKkrJNXhZ4K1kVWU3ZqDlx/gv59x9dH0ZjfnLuWKAM
#> vr1WpqOLUB5LRJIjR/4ksXK4ODLfOmAvnVjEk1eiFcJPzlnsrGuX5EGy6tlYWk+u
#> ypPZKu/Ih5UjgiSNepggdUn/RUrA6q/gak+eldHM/9StWZygwoIFmnXFt8/3U/bo
#> r+BOrCpcmMy5e+cdcJutgRuMRcMQ/VwkpYE7VpajfphxN6C+1W0+lMm/NtyM3lut
#> 6dTG7wIDAQABo1YwVDASBgNVHRMBAf8ECDAGAQH/AgEAMB0GA1UdDgQWBBTOttwM
#> NGzJdTcr+kINj4U0QiQQJTAfBgNVHSMEGDAWgBTOttwMNGzJdTcr+kINj4U0QiQQ
#> JTANBgkqhkiG9w0BAQsFAAOCAgEAji+t9l+sVZKrV2UxmQ1E1rzcL32s6rt+uk1H
#> Nc6G8wFlbIc0u8Z3xW/JFDMJw+Ia6zNoCgFTfRLQ8dg1v0b5cP/f+12P4lsPrRYV
#> VmcrFZPsHgC//QaMe3NH4+Z8Xo6vZxH8Fr9D8ikjTViVPngjX83XwKleNUjBh1Am
#> R16g09T17sjn93ffQN3GObLeMCu+BB0R/84qKZ6bczKTPovHRQnJ73V644jUm557
#> wr5NU7TS6szD1bwVsmuTc2/eSCS/qZOTuO7JMEi/iyuLBqeQDRhNFBSZoRVtZkl+
#> JECGPQcFxgGXqGvpQIB519/ACe/LTyBwMsIv62ryyR+gn0N4zkXdvwOlBVTxfaLe
#> M/OcS4Md7XGK/9AJFkrG9GpCq98YP3ENrQ6o3auG25M435oHvcvQ3efWJwjThQcS
#> Rp6ehDYSEar8ti2ct2wyud1KcEB5sSsr+yXesKHbhEJDqtHroplK0PiComGs1TWh
#> yc5h0/Tyw4BwruwTFOePTs9pDkdJn3LcRVMunEWnWJbEOIKc2+14tSxXx+q9lFTY
#> e8hVgulFBDjVwGve6qHtpbQLWkJGS4Qmc4XWUX+I8uQzPBwoMUkCOzdZur2kLNjn
#> sRp5cXyPOykwCX2feyFAer6nbYHHSWllDVcAD3fP0dcAP9MboWU4CE7yEaikGVvS
#> AX3kfqU=
#> -----END CERTIFICATE-----
#> ",""))'
```
