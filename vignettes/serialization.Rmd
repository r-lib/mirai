---
title: "Serialization - Arrow, polars, ADBC and torch"
vignette: >
  %\VignetteIndexEntry{Serialization - Arrow, polars, ADBC and torch}
...
2. [Torch Integration](#torch-integration)
...
daemons(1)
#> Error in daemons(1): could not find function "daemons"
everywhere(library(arrow))
#> Error in everywhere(library(arrow)): could not find function "everywhere"

x <- as_arrow_table(iris)

m <- mirai(list(a = head(x), b = "some text"), x = x)
#> Error in mirai(list(a = head(x), b = "some text"), x = x): could not find function "mirai"
m[]
#> Error: object 'm' not found

daemons(0)
#> Error in daemons(0): could not find function "daemons"
```
...
#> Error in serial_config("ArrowTabular", arrow::write_to_raw, function(x) arrow::read_ipc_stream(x, : could not find function "serial_config"

daemons(1, serial = cfg)
#> Error in daemons(1, serial = cfg): could not find function "daemons"
...
#> Error in daemons(n = 1, serial = serial_config(c("ArrowTabular", "RPolarsDataFrame"), : could not find function "daemons"
...
#> torch_tensor
#> (1,1,.,.) = 
#>  -0.0998  0.0766 -0.0904  0.0444 -0.1899
#>  -0.1116 -0.1426  0.1436 -0.0521 -0.1459
#>  -0.0699 -0.1158  0.1616  0.0032  0.1651
#>   0.0591  0.0624 -0.1588 -0.0790  0.1479
#>   0.1356 -0.0866 -0.0340 -0.0752 -0.1247
#> 
#> (2,1,.,.) = 
#>   0.0953  0.0754 -0.0067 -0.0671 -0.0116
#>   0.1441 -0.0282  0.0454 -0.0739 -0.1008
#>   0.1876  0.0302  0.0213 -0.1286 -0.0824
#>  -0.0981  0.0933  0.0915 -0.1830  0.0762
#>  -0.0084  0.1604  0.1021  0.1904 -0.1452
#> 
#> (3,1,.,.) = 
#>  -0.1287  0.1059  0.1762  0.1417  0.0228
#>   0.1608  0.0328  0.0704  0.1706  0.1738
#>  -0.0507 -0.1387  0.0387 -0.1842 -0.1448
#>  -0.1490 -0.1033 -0.0765  0.1360  0.0706
#>   0.0464  0.0050  0.1289 -0.1624  0.1443
#> 
#> (4,1,.,.) = 
#>  -0.1471 -0.1453 -0.0498 -0.0955  0.1568
#>  -0.0159  0.0701  0.1076 -0.1853  0.1883
#>   0.0102 -0.1490  0.1131  0.0303  0.1994
#>  -0.0539  0.1748  0.0168  0.1679  0.0859
#>   0.1161 -0.1593  0.0882  0.1584  0.1726
#> 
#> (5,1,.,.) = 
#>   0.0932  0.0720 -0.1765  0.0657 -0.1945
