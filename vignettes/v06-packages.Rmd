...
5. For CRAN packages, all examples and tests should respect the following rules when running on CRAN:
...
- Always reset daemons with `daemons(0)` at the end of each example or test file. Then allow at least a one-second sleep to ensure that all background processes have properly exited and that no 'detritus' from the processes remains.
