...
```
The key to adopting a flexible-use pattern that lets you scale up and down is to specify one of the following arguments to the `...` argument of `launch_local()` or `launch_remote()`. You may also supply these to the initial `daemons()` call for them to be used by default for all launches:
