...
* Successive `launch_remote()` commands are now identical under dispatcher - daemons automatically retrieve the next RNG stream from dispatcher and no longer require the `rs` argument to `daemon()`.
