...
3.  (Optional) Use
    [`everywhere()`](https://mirai.r-lib.org/reference/everywhere.md) to
    load `torch` on all daemons
...
[`daemons()`](https://mirai.r-lib.org/reference/daemons.md) call. The
class is ‘nanoarrow_array_stream’ since `nanoarrow` backs all DBI
`db*Arrow()` queries:
...
Use [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) to write or
query the database in Arrow format:
...
[`mirai()`](https://mirai.r-lib.org/reference/mirai.md) using the
