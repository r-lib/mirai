...
exists because `future(...)` alone isn’t always async - it blocks
whenever the number of tasks exceed available workers.
[`mirai()`](https://mirai.r-lib.org/reference/mirai.md) is built as an
async framework, so use it directly in place of
[`future_promise()`](https://rstudio.github.io/promises/reference/future_promise.html).
...
`future()` and
[`future_promise()`](https://rstudio.github.io/promises/reference/future_promise.html)
by default infer global variables. This means that they use heuristics
to try to determine which variables are needed for the expression, and
automagically passes them to the worker process.

If your code depended on this, instead pass variables explicitly via
`...` in [`mirai()`](https://mirai.r-lib.org/reference/mirai.md). A
mirai requires self-contained expressions with all variables or helper
functions explicitly supplied.

If your code used the `globals` argument, pass this directly to `.args`
in [`mirai()`](https://mirai.r-lib.org/reference/mirai.md).
...
abstraction](https://en.wikipedia.org/wiki/Leaky_abstraction), which can
lead to unpredictable edge cases or slower operation from sending
unnecessary data to daemons. Explicitly passing variables ensures
reliable and transparent behaviour. See [this article by Joe
Cheng](https://rstudio.github.io/promises/articles/promises_05a_futures.html#caveats-and-limitations)
for a full discussion.

**Capture globals using
[`environment()`](https://rdrr.io/r/base/environment.html):**

[`mirai()`](https://mirai.r-lib.org/reference/mirai.md) accepts an
...
provided to the ExtendedTask invoke method. See the Shiny vignette for
examples.
...
#> 
#> [[2]]
#> [1] 0.5623672 0.5822853 0.4782794
...
[`daemons()`](https://mirai.r-lib.org/reference/daemons.md) (see Random
Number Generation in the reference vignette).
...
Use [`everywhere()`](https://mirai.r-lib.org/reference/everywhere.md) to
call `devtools::load_all()` on all (local) daemons. They’ll then access
the same functions as your host session for subsequent
[`mirai()`](https://mirai.r-lib.org/reference/mirai.md) calls.

### 4. Why does `mirai()` take time when it’s meant to return immediately?

A [`mirai()`](https://mirai.r-lib.org/reference/mirai.md) call returns
almost instantaneously, as does Shiny `ExtendedTask`. The only reason it
takes time is passing large objects requiring serialization to the
parallel process.

Be careful passing functions or environments to
[`mirai()`](https://mirai.r-lib.org/reference/mirai.md) via `...` or
...
`...` in
[`launch_local()`](https://mirai.r-lib.org/reference/launch_local.md) or
[`launch_remote()`](https://mirai.r-lib.org/reference/launch_local.md).
Supply these to the initial
[`daemons()`](https://mirai.r-lib.org/reference/daemons.md) call to
